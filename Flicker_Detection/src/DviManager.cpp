...
uint8_t DviManager::consumer_worker(int channelId, int core)
{
    ...
    while (channel.is_running)
    {
        ...
        cv::putText(currentFrame, "Time: " + timeStr, cv::Point(10, 270),
                    cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
        cv::putText(currentFrame, "FPS: " + std::to_string(channel.fps.load()), cv::Point(10, 300),
                    cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
        cv::putText(currentFrame, "Frames: " + std::to_string(frameCounter), cv::Point(10, 330),
                    cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
        cv::putText(currentFrame, "Errors: " + std::to_string(errorCounter), cv::Point(10, 360),
        ...
        cv::putText(currentFrame, "Temp: " + std::to_string(roundedTemp) + " C",
                    cv::Point(10, 390), cv::FONT_HERSHEY_SIMPLEX, 0.7,
                    tempColor, 2);
